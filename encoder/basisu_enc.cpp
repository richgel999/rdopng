...
#define BASISU_USE_PACKAGEMERGE (1)

#if BASISU_USE_PACKAGEMERGE
...
#error TODO
...
#else
		// Call lodepng at a lower level to avoid extra allocations and memory copies.
		w = h = 0;
		unsigned char* buffer = nullptr;
		unsigned error = lodepng_decode_memory(&buffer, &w, &h, pBuf, buf_size, LCT_RGBA, 8);
		if (!buffer || error || !w || !h)
			return false;
						
		// Now pass ownership of the heap buffer directly to the image, to avoid having to copy it around.
		// This assumes lodepng allocates the buffer using malloc(), which it does.
		img.grant_ownership(reinterpret_cast<color_rgba *>(buffer), w, h);
#endif
	...
	bool save_png(uint8_vec &output_buf, const image& img, uint32_t image_save_flags, uint32_t grayscale_comp, int filter_index, const uint8_t* pFilters, uint64_t *pFile_size)
	{
		output_buf.resize(0);
		...
		if (filter_index != -1)
		{
			...
			filters.resize(img.get_height());
			filters.set_all((uint8_t)filter_index);

			state.encoder.predefined_filters = filters.data();
		}
		else if (pFilters)
		{
			state.encoder.filter_strategy = LFS_PREDEFINED;

			state.encoder.predefined_filters = pFilters;
		}
		...
		if (image_save_flags & cImageSaveGrayscale)
		{
			...
			err = lodepng_encode(&pOut, &outsize, g_pixels.data(), img.get_width(), img.get_height(), &state);
		...
		else
		{
			...
			if ((!has_alpha) || ((image_save_flags & cImageSaveIgnoreAlpha) != 0))
			{
				...
				err = lodepng_encode(&pOut, &outsize, rgb_pixels.data(), img.get_width(), img.get_height(), &state);
				lodepng_state_cleanup(&state);
			}
			else
			{
				...
				err = lodepng_encode(&pOut, &outsize, (const uint8_t*)img.get_ptr(), img.get_width(), img.get_height(), &state);
				lodepng_state_cleanup(&state);
			}
		}
		...
		output_buf.resize(outsize);
		memcpy(output_buf.data(), pOut, outsize);
	...
	bool save_png(const char* pFilename, const image& img, uint32_t image_save_flags, uint32_t grayscale_comp, int filter_index, const uint8_t* pFilters, uint64_t* pFile_size)
	{
		uint8_vec output_buf;

		if (!save_png(output_buf, img, image_save_flags, grayscale_comp, filter_index, pFilters, pFile_size))
			return false;

		if (!write_vec_to_file(pFilename, output_buf))
			return false;
	...
	bool huffman_encoding_table::init(uint32_t num_syms, const uint16_t *pFreq, uint32_t max_code_size)
	{
...
#else
		std::vector<uint32_t> freq32(num_syms);
		for (uint32_t i = 0; i < num_syms; i++)
			freq32[i] = pFreq[i];
		...
		unsigned char actualMaxLen = packageMerge((uint8_t)max_code_size, num_syms, freq32.data(), m_code_sizes.data());
		if (!actualMaxLen)
			return false;
